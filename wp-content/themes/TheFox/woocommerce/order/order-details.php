...
?>
<h2><?php _e( 'Order Details', 'thefoxwp' ); ?></h2>
							...
							<?php
								...
								if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
									...
									foreach ( $download_files as $download_id => $file ) {
										...
										$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file%s', 'thefoxwp' ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
									}
	...
	<?php
		...
		if ( $totals = $order->get_order_item_totals() ) {
			foreach ( $totals as $key => $total ) {
				...
				if ( $has_refund && $key === 'order_total' ) {
					...
					if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
						...
						if ( ! empty( $tax_del_array ) ) {
							$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', 'thefoxwp' ), implode( ', ', $tax_del_array ) );
						}

						if ( ! empty( $tax_ins_array ) ) {
							$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', 'thefoxwp' ), implode( ', ', $tax_ins_array ) );
						}
					}
				...
				?>
				<tr>
					<th scope="row"><?php echo !empty( $total['label'] ) ? $total['label'] : ''; ?></th>
		...
		if ( $has_refund ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Refunded:', 'thefoxwp' ); ?></th>
		...
		if ( '' != $order->customer_note ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Note:', 'thefoxwp' ); ?></th>
...
<div class="complete_customer_ctn">
	<h2><?php _e( 'Customer details', 'thefoxwp' ); ?></h2>

<dl class="customer_details">
<?php
	if ( $order->billing_email ) echo '<dt>' . __( 'Email:', 'thefoxwp' ) . '</dt><dd>' . $order->billing_email . '</dd>';
	if ( $order->billing_phone ) echo '<dt>' . __( 'Telephone:', 'thefoxwp' ) . '</dt><dd>' . $order->billing_phone . '</dd>';
...
<?php endif; ?>
		...
		<address><p>
			<?php
				if ( ! $order->get_formatted_billing_address() ) _e( 'N/A', 'thefoxwp' ); else echo ''.$order->get_formatted_billing_address();
			?>
		</p></address>
...
<?php if ( ! wc_ship_to_billing_address_only() && $order->needs_shipping_address() && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) : ?>
	...
	<div class="rd_customer_shipping_adress">

			<h2><?php _e( 'Shipping Address', 'thefoxwp' ); ?></h2>
		<address><p>
			<?php
				if ( ! $order->get_formatted_shipping_address() ) _e( 'N/A', 'thefoxwp' ); else echo ''.$order->get_formatted_shipping_address();
			?>
		</p></address>
