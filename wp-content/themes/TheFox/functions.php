...
function rd_register_menu() {
	...
	register_nav_menu('mobile-menu', __('Mobile Menu','thefoxwp'));
}
    ...
    if ($count > 0) {
        ...
        if (is_array($terms)) {
            foreach ($terms as $term) {
                ...
                $term_list .= '><a href="#filter" data-option-value=".sf_' . $term->slug . '" title="' . sprintf(__('View all post filed under %s', 'thefoxwp'), $term->name) . '">' . $term->name . '</a>
                </li>';
    ...
    if ($count > 0) {
        ...
        if (is_array($terms)) {
            foreach ($terms as $term) {
                ...
                $tempname = strtolower($tempname);
                $term_list .= '><a href="#filter" data-option-value=".' .$term->slug . '" title="' . sprintf(__('View all post filed under %s', 'thefoxwp'), $term->name) . '">' . $term->name . '</a>
                </li>';
                if ($count != $i) $term_list .= ' '; else $term_list .= '';
                if ($iterm<$termcount) {$term_list .= '';}
                $iterm++;
