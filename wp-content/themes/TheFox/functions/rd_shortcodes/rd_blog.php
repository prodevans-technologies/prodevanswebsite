...
function blog_sc($atts, $content = null)
        {
            extract(shortcode_atts(array(
				...
				'blog_hl_color' => '',
				'blog_hover_color' => '',
				'blog_border_color' => '#ebebeb',
				'blog_timelinedb_color' => '',
				'blog_v_color' => '',
				'blog_v_alt_color' => '',
				'blog_categorybg_color' => '#f6f7f8',
				'blog_category_color' => '#999999',
...
//business blog
$custom_blog_css .= '.business_author_post .post_b_author a{color:'.$blog_heading_color.'; }.business_author_post .post_b_date{color:'.$blog_category_color.'; }';
$custom_blog_css .= '.business_author_post .entry,.business_author_post .entry p{color:'.$blog_text_color.'; }';
$custom_blog_css .= '.business_author_post .post_b_category a{background:'.$blog_categorybg_color.'; border:1px solid '.$blog_border_color.'; color:'.$blog_category_color.'; }';
$custom_blog_css .= '.business_author_post .post-info a:first-child{color:'.$blog_hl_color.'; }.business_author_post .post-info a:hover:first-child{color:'.$blog_heading_color.'; }.business_author_post .post-info a,.business_author_post .post-info{color:'.$blog_category_color.';}.business_author_post .post-info a:hover,.business_author_post .post_b_author a:hover{color:'.$blog_hover_color.';}.business_author_post .post_quote_author,.business_author_post .post_quote_text:after,.business_author_post .post_quote_text{background:'.$blog_hl_color.'; color:'.$blog_bg_color.';}';
                ...
                function get_blog_posts (this_obj) {
                    ...
                    $.ajax({
                        ...
                        success: function(result){
	                            ...
	                            if(result.length<1){
                                $("#rd_<?php echo esc_js($id); ?> .blog_load_more_cont a").html("<?php echo __('No more posts','thefoxwp'); ?>");
	                            }
...
<?php } ?>
<div id="rd_<?php echo esc_attr($id); ?>" class="masonry_ctn <?php  if ($type == 'type2') { echo 'blog_timeline';  }elseif($type == 'type4' || $type == 'type6') { echo 'grid_post';  }elseif($type == 'type3') { echo 'blog_masonry';  }elseif($type == 'type7') { echo 'standard_trending';  }elseif($type == 'type8') { echo 'standard_business';  }if($type == 'type6' || $type == 'type7' ) { echo ' trend_loadmore';  } if($blog_navigation !== 'classic_nav'){ echo ' ajax_blog';  } ?>">
<?php 
...
<?php  echo rd_custom_excerpt('rd_port_long_excerpt','rd_trend_more'); ?>
    ...
    <?php the_author_posts_link(); if( function_exists('zilla_likes') ){ echo do_shortcode('[zilla_likes]');	} ?>
    </div>
     </div>
     </div>
	</div>
...
if(get_post_meta($post->ID, 'rd_show_slider', true) == 'yes') {
 ...
 ?>
  <div class="post_ctn"> 
    ...
    <div class="post-info"><?php echo __('By','thefoxwp') ?> <?php the_author_posts_link(); ?>    |    <?php the_category(', '); ?>    |    <?php comments_popup_link(__('0 comment','thefoxwp'),__('1  comment','thefoxwp'),__('% comments','thefoxwp'),'comments-link',__('Comments are Closed','thefoxwp')); ?>    |    <?php if( function_exists('zilla_likes') ){ echo do_shortcode('[zilla_likes]');	}?></div>
...
<!-- .title END--> 
<!-- post-info-top -->
<div class="post-info"><?php echo __('By','thefoxwp') ?> <?php the_author_posts_link(); ?>    |    <?php the_time('j F, Y') ?></div>
<!-- post-info END--> 
<!-- .entry -->
<div class="entry">
      <?php 	if ( $post_format == 'audio' ){ echo !empty( $content ) ? $content : '';}else{ the_content(__('Read more', 'thefoxwp')); }?>
...
<div class="navigation clearfix <?php if($type == 'type8') { echo esc_attr('b_nav'); }?>">
