...
$html = ( $style !== 'vc_box_shadow_3d' && ($hover_effect == 'img_zoom_effect' || $hover_effect == 'img_play_effect' )) ? '<span>' . $img['thumbnail'] . '</span>' : $img['thumbnail'];
...
$he_t .=	'<a' . $a_class . ' href="' . $link_to . '"' . ' target="' . $img_link_target . '"'. '>';
}
...
if($hover_effect == 'img_hover_title'){
$html = '<div class="vc_single_image-wrapper ' . $style . ' ' . $border_color . '">'. $he_t .' '. $html . '</div>';	
}else{
...
$class_to_filter = 'wpb_single_image wpb_content_element vc_align_' . $alignment . ' ' . $animation . ' ' .$hover_effect ; 
